...
/**
 * Formats all lClasses in a learning class repository.
 * @param root The root of the learning class repository.
**/

def root = '/temp/'

formatAllLClasses (root)

def void formatAllLClasses (String root) {
    def schemas = getSchemas (root)
    for (schema in schemas)
        formatSchema ('file://'+ schema, ' ', ' ')
}

def void formatSchema (uri, prefix, namespace) {
...
def List getSchemas (String rootPath) {
    def schemas = [] 
