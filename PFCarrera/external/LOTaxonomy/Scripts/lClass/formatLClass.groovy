...
/**
 * Formats a lClass.
 * @param uri The uri of xml schema.
 * @param targetNS The new namespace.
 * @param prefixNS The prefix of the namespace.
**/

def uri = 'file:///temp/schema3.xsd'
def targetNS = 'http://foo.com/schema'
def prefixNS = 'pr'
formatLClass (uri, prefixNS, targetNS)

def void formatLClass (uri, prefix, namespace) {
   document = loadDocument (uri)
   document = setTargetNamespace (document, prefix, namespace)
   xml = formatXML (document)
   saveDocument (uri, xml)
   
   println 'lClass ' + uri + ' succsessfully formated'
}
